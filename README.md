...
The `rust_core` crate contains the core implementation and is built independently. Use feature flags to compile Python or Node.js bindings.  All
native extensions live under the `bindings/` directory:
* `bindings/python` contains the thin Python wrapper used by tests and benchmarks.
* `bindings/nodejs` exposes the same Rust functions for Node.js.
...
A GitHub Actions workflow using **uv** installs dependencies, builds the Rust wheel and runs the unit tests.
